...
DDEESSCCRRIIPPTTIIOONN
       ...
       up _h_o_o_k_s there. See documentation for ggiitt--rreecceeiivvee--ppaacckk(1).

       When the command line does not specify where to push with the <<rreeppoossiittoorryy>> argument,
       bbrraanncchh..**..rreemmoottee configuration for the current branch is consulted to determine where to push.
       ...
       configuration, and if it is not found, honors ppuusshh..ddeeffaauulltt configuration to decide what to
       push (See ggiitt--ccoonnffiigg(1) for the meaning of ppuusshh..ddeeffaauulltt).

       When neither the command-line nor the configuration specify what to push, the default behavior
       is used, which corresponds to the ssiimmppllee value for ppuusshh..ddeeffaauulltt: the current branch is pushed
...
OOPPTTIIOONNSS
       ...
       <refspec>...
           ...
           rreeffss//** on the destination <repository> it belongs based on the type of <src> being pushed
           ...
           Whether this is allowed depends on where in rreeffss//** the <dst> reference lives as described
           ...
           The rreeffss//ttaaggss//** namespace will accept any kind of object (as commits, trees and blobs can
           be tagged), and any updates to them will be rejected.

           It’s possible to push any type of object to any namespace outside of rreeffss//{{ttaaggss,,hheeaaddss}}//**.
           ...
           cases where what’s being fast-forwarded is not a commit, but a tag object which happens to
           point to a new commit which is a fast-forward of the commit the last tag (or commit) it’s
           replacing. Replacing a tag with an entirely different tag is also allowed, if it points to
           the same commit, as well as pushing a peeled tag, i.e. pushing the commit that existing
           tag object points to, or a new tag object which an existing commit points to.
           ...
           forbidden by configuration or hooks. See rreecceeiivvee..ddeennyyDDeelleetteess in ggiitt--ccoonnffiigg(1) and
       ...
       --prune
           ...
           ttmmpp will be removed if a local branch with the same name doesn’t exist any more. This also
           respects refspecs, e.g.  ggiitt ppuusshh ----pprruunnee rreemmoottee rreeffss//hheeaaddss//**::rreeffss//ttmmpp//** would make sure
           that remote rreeffss//ttmmpp//ffoooo will be removed if rreeffss//hheeaaddss//ffoooo doesn’t exist.
       ...
       --[no-]signed, --signed=(true|false|if-asked)
           ...
           by the hooks and/or be logged. If ffaallssee or ----nnoo--ssiiggnneedd, no signing will be attempted. If
           ttrruuee or ----ssiiggnneedd, the push will fail if the server does not support signed pushes. If set
           to iiff--aasskkeedd, sign if and only if the server supports signed pushes. The push will also
           fail if the actual call to ggppgg ----ssiiggnn fails. See ggiitt--rreecceeiivvee--ppaacckk(1) for the details on
           the receiving end.
       ...
       -o <option>, --push-option=<option>
           ...
           order listed on the command line. When no ----ppuusshh--ooppttiioonn==<<ooppttiioonn>> is given from the command
       ...
       --[no-]force-with-lease, --force-with-lease=<refname>, --force-with-lease=<refname>:<expect>
           ...
           pushing with ----ffoorrccee will lose her work.
           ...
           <<eexxppeecctt>> (which is allowed to be different from the remote-tracking branch we have for the
           refname, or we do not even have to have such a remote-tracking branch when this form is
           used). If <<eexxppeecctt>> is the empty string, then the named ref must not already exist.
           ...
           updating refs in the background. We don’t have anything except the remote tracking info to
           go by as a heuristic for refs you’re expected to have seen & are willing to clobber.
           ...
           Now when the background process runs ggiitt ffeettcchh oorriiggiinn the references on oorriiggiinn--ppuusshh won’t
           ...
           defeated by something that runs ggiitt ffeettcchh ----aallll, in that case you’d need to either disable
           ...
           I.e. create a bbaassee tag for versions of the upstream code that you’ve seen and are willing
           to overwrite, then rewrite history, and finally force push changes to mmaasstteerr if the remote
           version is still at bbaassee, regardless of what your local rreemmootteess//oorriiggiinn//mmaasstteerr has been
       ...
       -f, --force
           ...
           rreemmoottee..**..ppuusshh may overwrite refs other than the current branch (including local refs that
           are strictly behind their remote counterpart). To force a push to only one branch, use a ++
           in front of the refspec to push (e.g ggiitt ppuusshh oorriiggiinn ++mmaasstteerr to force a push to the mmaasstteerr
           branch). See the <<rreeffssppeecc>>......  section above for details.
       ...
       -u, --set-upstream
           ...
           reference, used by argument-less ggiitt--ppuullll(1) and other commands. For more information, see
       ...
       --no-recurse-submodules, --recurse-submodules=check|on-demand|only|no
           ...
           status. If _o_n_-_d_e_m_a_n_d is used all submodules that changed in the revisions to be pushed
           will be pushed. If on-demand was not able to push all necessary revisions it will also be
           aborted and exit with non-zero status. If _o_n_l_y is used all submodules will be recursively
           pushed while the superproject is left unpushed. A value of _n_o or using
           ----nnoo--rreeccuurrssee--ssuubbmmoodduulleess can be used to override the push.recurseSubmodules configuration
...
GGIITT UURRLLSS
       ...
       _g_i_t _c_l_o_n_e, _g_i_t _f_e_t_c_h and _g_i_t _p_u_l_l, but not _g_i_t _p_u_s_h, will also accept a suitable bundle file.
       See ggiitt--bbuunnddllee(1).
...
OOUUTTPPUUTT
       ...
       summary
           ...
           form suitable for using as an argument to ggiitt lloogg (this is <<oolldd>>....<<nneeww>> in most cases, and
           ...
           remote rejected
               ...
               rreecceeiivvee..ddeennyyNNoonnFFaassttFFoorrwwaarrddss (for forced non-fast-forward updates), rreecceeiivvee..ddeennyyDDeelleetteess
               or rreecceeiivvee..ddeennyyDDeelleetteeCCuurrrreenntt. See ggiitt--ccoonnffiigg(1).
...
EEXXAAMMPPLLEESS
       ggiitt ppuusshh
           Works like ggiitt ppuusshh <<rreemmoottee>>, where <remote> is the current branch’s remote (or oorriiggiinn, if
       ...
       ggiitt ppuusshh mmootthheerrsshhiipp mmaasstteerr::ssaatteelllliittee//mmaasstteerr ddeevv::ssaatteelllliittee//ddeevv
           ...
           and run ggiitt mmeerrggee there to complete the emulation of ggiitt ppuullll that were run on mmootthheerrsshhiipp
           to pull changes made on ssaatteelllliittee.
...
SSEECCUURRIITTYY
       ...
       protect from a malicious peer, your best option is to store it in another repository. This
       ...
       The known attack vectors are as follows:
        ...
        1. The victim sends "have" lines advertising the IDs of objects it has that are not
           explicitly intended to be shared but can be used to optimize the transfer if the peer also
           has them. The attacker chooses an object ID X to steal and sends a ref to X, but isn’t
           required to send the content of X because the victim already has it. Now the victim
